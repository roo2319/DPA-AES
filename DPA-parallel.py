...
def attack( argc, argv ):
  ...
  Traces = T[:,:samples]
  pargs = enumerate([numpy.array(H[:,byte,:]) for byte in range(M.shape[1])])
  pargs = map(lambda x: x + (Traces,),pargs)
  pool = multiprocessing.Pool(processes=16)
  results = pool.starmap(calculateKeyByte,pargs)
  pool.close()
  pool.join()
  print(results)
