...
def calculateKeyByte(idx,Hyp,Traces,bounds):
  print("Calculating byte {}".format(idx))
  sys.stdout.flush()

  R = numpy.zeros((Hyp.shape[1],Traces.shape[1]))
  for t_index in range(bounds[0],bounds[1]):
    trace = Traces[:,t_index] # tc elements
    if t_index %1000 == 0:
          print("Reached t = ",t_index)
          print(numpy.unravel_index(numpy.argmax(R),R.shape), numpy.max(R) )
    for k in range(Hyp.shape[1]):
      keyData = Hyp[:,k] #t elements
      cor, _ = pearsonr(keyData,trace)
      R[k,t_index] = 0 if numpy.isnan(cor) else abs(cor)
  
  print("Byte {}: Max R at {}, with correlation {}".format(idx,numpy.unravel_index(numpy.argmax(R),R.shape), numpy.max(R)))
  return idx,numpy.unravel_index(numpy.argmax(R),R.shape), numpy.max(R) 
...
def attack( argc, argv ):
  ...
  bounds = (0,samples)
  results = []
  first = calculateKeyByte(0,H[:,0,:],T,bounds)
  bounds = (first[1][1],bounds[1])
  results.append(first)
  last = calculateKeyByte(15,H[:,15,:],T,bounds)
  bounds = (bounds[0],last[1][1])
  for i in range(1,15):
      n = calculateKeyByte(i,H[:,i,:],T,bounds)
      bounds = (n[1][1],bounds[1])
      results.append(n)
  results.append(last)
  print(results)
